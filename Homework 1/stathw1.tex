...
\begin{enumerate}
\item For Dataset 1, the distribution is skewed left, as seen in the qq plot. The distribution does not seem normal and is evident from the proof that mean(0.998600) is far from median(0.692800). The distribution does not show any kind of symmetry.
\item For dataset 2, the distribution seems to be normal, this can be inferred from near straight line in qqplot. The distribution seems to suffer from fat tailed distribution. Since this is a near-normal distribution mean(50) is near to median(49.44). There seems to be a symmetry through the median, however this is not perfect. This goes with the discussion in the class that the practical data sets and models seldom match with the perfect theoretical models and assumptions.
\end{enumerate}
